...
int main(int argc, char** argv)
{
	...
	//Instantiate the database - The database contains all
	//database connections and post calls
    Db database;
    ...
    if (!programWindow.create(windowWidth, windowHeight, windowBPP, windowFullscreen))
    {
        //If it fails
#ifdef _WIN32
        MessageBoxW(NULL, L"Unable to create the OpenGL Window", L"An error occurred", MB_ICONERROR | MB_OK);
#endif
    ...
    {
#ifdef _WIN32
        MessageBoxW(NULL, L"Could not initialize the application", L"An error occurred", MB_ICONERROR | MB_OK);
#endif
        programWindow.destroy(); //Reset the display and exit
        return 1;
    }
